using CommunityToolkit.Mvvm.ComponentModel;using Firebase.Auth;using Newtonsoft.Json;using System;using System.Collections.Generic;using System.Linq;using System.Text;using System.Threading.Tasks;using ValetAcounting.View;namespace ValetAcounting.ViewModel{    public partial class LoginViewModel : BaseViewModel    {        public string webApiKey = "AIzaSyBeUx58Kzv8LsT-OliZTeCBuAIq7TuB6WU";        private INavigation _navigation;        public Command RegisterBtn { get; }        public Command LoginBtn { get; }        [ObservableProperty]        private string userName;        [ObservableProperty]        private string userPassword;        public LoginViewModel()        {
            ...
            RegisterBtn = new Command(RegisterBtnTappedAsync);            LoginBtn = new Command(LoginBtnTappedAsync);            string passwordFromPrefs = Preferences.Get("UserPassword", null);            string usernameFromPrefs = Preferences.Get("UserName", null);            if (passwordFromPrefs != null)            {                UserPassword = passwordFromPrefs;                UserName = usernameFromPrefs;                LoginBtnTappedAsync(null);            }        }        private async void LoginBtnTappedAsync(object obj)        {            var authProvider = new FirebaseAuthProvider(new FirebaseConfig(webApiKey));            try            {                UserName = UserName.Trim();                UserPassword = UserPassword.Trim();                var auth = await authProvider.SignInWithEmailAndPasswordAsync(UserName, UserPassword);                var content = await auth.GetFreshAuthAsync();                var serializedContent = JsonConvert.SerializeObject(content);                Preferences.Set("FreshFirebaseToken", serializedContent);                Preferences.Set("UserPassword", UserPassword); // The first parameter is the key
                Preferences.Set("UserName", UserName); // The first parameter is the key
